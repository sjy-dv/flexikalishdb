...
func (f *decompressor) Close() error {
	...
	byteReader := bytes.NewReader(f.dict.hist)
	read := 0
	...
	for {
		...
		if len(f.dict.hist) < aead.NonceSize() {
			return errors.New("ciphertext too short")
		}

		// Split nonce and ciphertext.
		nonce, ciphertext := f.dict.hist[:aead.NonceSize()], f.dict.hist[aead.NonceSize():]

		opnd, err := aead.Open(nil, nonce, ciphertext, nil)
		if err != nil {
			return err
		}

		bytes.Replace(f.dict.hist, byteStorage[:nn], opnd, nn)
...
func NewReader(r io.Reader, key []byte) io.ReadCloser {
